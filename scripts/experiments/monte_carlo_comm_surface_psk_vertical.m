...
result_file = 'monte_carlo_comm_surface_psk_vertical_result.mat';
figure_prefix = 'monte_carlo_comm_surface_psk_vertical_';
if exist(result_file, 'file')
    warning('Result file %s already exists and will be overwritten.', result_file);
end

params_base = local_base_params();
sea_conditions = local_sea_conditions();
...
EbN0_dB_list = comm_cfg.EbN0_dB_list;

mc_count = 4;
mc_override = str2double(getenv('SURFACE_COMM_MC_COUNT'));
...
for cc = 1:n_cond
    for ss = 1:n_scen
        for mm = 1:mc_count
            ...
            fprintf(['Comm MC condition %d/%d (%s), scenario %d/%d (%s), ' ...
                'seed %d/%d, sea_seed=%d\n'], ...
            ...
            if invariant_error > 1e-10
                error('monte_carlo_comm_surface_psk_vertical:Invariant', ...
            ...
            run_result = struct();
            run_result.condition_index = cc;
            run_result.condition_name = sea_conditions(cc).name;
            run_result.scenario_index = ss;
            run_result.scenario_name = scenarios(ss).name;
            run_result.seed_index = mm;
            run_result.sea_seed = seed_list(mm);
            run_result.sea_hs_target = sea_conditions(cc).sea_hs_target;
            run_result.sea_wind_speed = sea_conditions(cc).sea_wind_speed;
            run_result.enable_surface_reflection = scenarios(ss).enable_surface_reflection;
            ...
            run_result.ebn0_reference = comm_cfg.ebn0_reference;
...
local_plot_condition_compare(curve_stats, sea_conditions, scenarios, 'BER', ...
    'direct_plus_reflect', 'BER: weak vs strong direct+reflect', ...
    figure_prefix, 'BER_weak_strong_direct_plus_reflect');
local_plot_condition_compare(curve_stats, sea_conditions, scenarios, 'SER', ...
    'direct_plus_reflect', 'SER: weak vs strong direct+reflect', ...
    figure_prefix, 'SER_weak_strong_direct_plus_reflect');
local_plot_scenario_compare(curve_stats, sea_conditions, scenarios, 'BER', ...
    'weak', 'BER: weak sea direct-only vs direct+reflect', ...
    figure_prefix, 'BER_weak_direct_vs_reflect');
local_plot_scenario_compare(curve_stats, sea_conditions, scenarios, 'BER', ...
    'strong', 'BER: strong sea direct-only vs direct+reflect', ...
    figure_prefix, 'BER_strong_direct_vs_reflect');
local_plot_scenario_compare(curve_stats, sea_conditions, scenarios, 'SER', ...
    'weak', 'SER: weak sea direct-only vs direct+reflect', ...
    figure_prefix, 'SER_weak_direct_vs_reflect');
local_plot_scenario_compare(curve_stats, sea_conditions, scenarios, 'SER', ...
    'strong', 'SER: strong sea direct-only vs direct+reflect', ...
    figure_prefix, 'SER_strong_direct_vs_reflect');

save(result_file, 'params_base', 'comm_cfg', 'sea_conditions', 'scenarios', ...
    'seed_list', 'bits_tx', 'EbN0_dB_list', 'ber_samples', 'ser_samples', ...
    'effective_snr_samples', 'run_results', 'run_summary_table', ...
...
function sea_conditions = local_sea_conditions()
sea_conditions = struct([]);
sea_conditions(1).name = 'weak';
sea_conditions(1).sea_hs_target = 0.05;
sea_conditions(1).sea_wind_speed = 5;
sea_conditions(2).name = 'strong';
sea_conditions(2).sea_hs_target = 1.0;
sea_conditions(2).sea_wind_speed = 5;
end
...
% samples has dimensions [EbN0 x mc_count x scenario x condition].
stats = struct();
stats.samples = samples;
stats.mean = squeeze(mean(samples, 2, 'omitnan'));
stats.var = squeeze(mean((samples - mean(samples, 2, 'omitnan')).^2, 2, 'omitnan'));
...
% mean_curves has dimensions [EbN0 x scenario x condition].
