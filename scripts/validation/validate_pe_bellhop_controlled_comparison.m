...
%
% P0 and Stages 0--7 are validation-only entry points. Stage 2 is unlocked
...
switch lower(char(stage))
    ...
    case 'stage5'
        result = local_stage5(cfg, root);
    case 'stage6'
        result = local_stage6(cfg, root);
    case 'stage7'
        result = local_stage7(cfg, root);
    otherwise
...
cfg = struct( ...
    ...
    'stage5_output_subdir', 'stage5_phase_attribution', ...
    'stage5_geometry_tolerance', 1e-6, ...
    'stage6_output_subdir', 'stage6_fixed_pm', ...
    'stage6_seed', 260001, ...
    'stage7_output_subdir', 'stage7_historical_m50_interpretation', ...
    'stage7_historical_dir', fullfile(root,'results','validation', ...
        'pe_bellhop_pm_model_discrepancy_statistics'), ...
    'stage7_historical_report', fullfile(root,'reports', ...
        'pe_bellhop_pm_model_discrepancy_statistics_report.md'), ...
    'canonical_coeff_source', fullfile(root, 'results', 'validation', ...
...
if isempty(cfg.official_exe)
    error('Set BELLHOP_EXE or overrides.official_exe to the AcousticsToolbox 2020 bellhop.exe.');
end
end
...
rows=repmat(struct('surface_type',"sinusoid",'A_m',0,'K_radpm',0,'E_G_M99',0,'E_aligned',0, ...
...
for ii=1:numel(cases)
    ...
    rows(ii)=struct('surface_type',"sinusoid",'A_m',c.A_m,'K_radpm',c.K_radpm,'E_G_M99',m.l2_m99, ...
...
stage6_file=fullfile(cfg.output_dir,cfg.stage6_output_subdir,'stage6_validation.mat');
pm_included=false;stage6_consistent=true;
if exist(stage6_file,'file')==2
    q=load(stage6_file,'validation');s6=q.validation;stage6_consistent=s6.passed;
    if s6.passed
        m=s6.metrics;pmrow=struct('surface_type',"fixed_PM",'A_m',NaN,'K_radpm',NaN, ...
            'E_G_M99',m.l2_m99,'E_aligned',m.aligned_l2_m99,'TL_RMS_dB',m.tl_rms_m99, ...
            'phase_RMS_rad',m.phase_rms_m99,'phi0_rad',m.global_phase_rad, ...
            'rho_raw',m.rho_raw,'rho_shape',m.rho_shape,'region',s6.validity_region, ...
            'numerical_mapping_guards',s6.checks.all);
        t=[t;struct2table(pmrow)];pm_included=true;
    end
end
height_slice=t(t.surface_type=="sinusoid"&abs(t.K_radpm-cfg.stage2_wavenumber_radpm)<1e-12,:);
slope_slice=t(t.surface_type=="sinusoid"&abs(t.A_m-s3.fixed_A_m)<1e-12,:);
intervals=struct( ...
...
checks=struct('stage1_pass',s1.passed,'stage2_pass',s2.passed,'stage3_pass',s3.passed, ...
    ...
    'stage6_consistent_if_present',stage6_consistent);
checks.all=all(structfun(@(v)logical(v),checks));
...
fprintf(fid,'| surface | A (m) | K (rad/m) | E_G | E_aligned | TL RMS (dB) | phase RMS (rad) | phi0 | rho_raw | rho_shape | region | guards |\n');
fprintf(fid,'|:---|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---:|:---:|\n');
for ii=1:height(v.case_table)
    r=v.case_table(ii,:);
    fprintf(fid,'| %s | %.9g | %.9g | %.9g | %.9g | %.9g | %.9g | %.9g | %.9g | %.9g | %s | %s |\n', ...
        char(r.surface_type),r.A_m,r.K_radpm,r.E_G_M99,r.E_aligned,r.TL_RMS_dB,r.phase_RMS_rad, ...
...
function result=local_stage5(cfg,root)
% Stage 5: read-only phase-discrepancy attribution for representative
% Region-II/III sinusoid cases. No solver or production physics is called.
base=cfg.output_dir;
files={fullfile(base,cfg.stage1_output_subdir,'stage1_validation.mat'), ...
    fullfile(base,cfg.stage2_output_subdir,'stage2_validation.mat'), ...
    fullfile(base,cfg.stage3_output_subdir,'stage3_validation.mat'), ...
    fullfile(base,cfg.stage4_output_subdir,'stage4_validation.mat'), ...
    fullfile(base,'stage0','stage0_validation.mat')};
for f=files
    if exist(f{1},'file')~=2,error('Stage 5 requires %s.',f{1});end
end
q=load(files{1},'validation');s1=q.validation;q=load(files{2},'validation');s2=q.validation;
q=load(files{3},'validation');s3=q.validation;q=load(files{4},'validation');s4=q.validation;
q=load(files{5},'validation');s0=q.validation;
if ~s1.passed||~s2.passed||~s3.passed||~s4.passed
    error('Stage 5 requires PASS results from Stages 1--4.');
end

t=s4.case_table; nonI=t(t.region~="I",:);
height_nonI=nonI(abs(nonI.K_radpm-cfg.stage2_wavenumber_radpm)<1e-12,:);
if isempty(height_nonI),error('No Region-II/III height cases are available.');end
spec=[height_nonI.A_m(1),cfg.stage2_wavenumber_radpm; ...
    height_nonI.A_m(end),cfg.stage2_wavenumber_radpm; ...
    s3.fixed_A_m,max(s3.planned_K_radpm)];
spec=unique(spec,'rows','stable');
cases=cell(1,size(spec,1));
for ii=1:size(spec,1)
    A=spec(ii,1);K=spec(ii,2);c=local_stage5_find_case(s2,s3,A,K);
    stationary=local_stage5_stationary_geometry(A,K,s0.x_m,cfg,s0.footprint);
    diagnostics=local_stage5_phase_diagnostics(c,stationary,s0.footprint,cfg);
    classification=local_stage5_classification(c.metrics);
    checks=struct('source_case_guards',local_stage5_case_guards(c), ...
        'finite',diagnostics.finite&&stationary.finite, ...
        'stationarity',stationary.max_stationarity_residual_m99<=cfg.stage5_geometry_tolerance, ...
        'specular_direction',stationary.max_specular_direction_error_m99<=cfg.stage5_geometry_tolerance);
    checks.all=all(structfun(@(v)logical(v),checks));
    cases{ii}=struct('A_m',A,'K_radpm',K,'region', ...
        t.region(abs(t.A_m-A)<1e-12&abs(t.K_radpm-K)<1e-12), ...
        'metrics',c.metrics,'classification',classification, ...
        'stationary_geometry',stationary,'phase_diagnostics',diagnostics, ...
        'checks',checks);
end
rows=repmat(struct('A_m',0,'K_radpm',0,'region',"",'classification',"", ...
    'E_G',0,'E_aligned',0,'alignment_reduction_fraction',0,'phi0_rad',0, ...
    'rho_raw',0,'rho_shape',0,'phase_RMS_rad',0,'TL_RMS_dB',0, ...
    'phase_gradient_rms_radpm',0,'phase_gradient_max_abs_radpm',0, ...
    'PE_stationary_phase_RMS_rad',0,'BH_stationary_phase_RMS_rad',0, ...
    'two_k_vs_angle_phase_RMS_rad',0,'stationarity_residual_max',0, ...
    'specular_direction_error_max',0,'guards_passed',false),1,numel(cases));
for ii=1:numel(cases)
    c=cases{ii};m=c.metrics;d=c.phase_diagnostics;g=c.stationary_geometry;
    rows(ii)=struct('A_m',c.A_m,'K_radpm',c.K_radpm,'region',c.region, ...
        'classification',c.classification,'E_G',m.l2_m99,'E_aligned',m.aligned_l2_m99, ...
        'alignment_reduction_fraction',1-m.aligned_l2_m99/m.l2_m99, ...
        'phi0_rad',m.global_phase_rad,'rho_raw',m.rho_raw,'rho_shape',m.rho_shape, ...
        'phase_RMS_rad',m.phase_rms_m99,'TL_RMS_dB',m.tl_rms_m99, ...
        'phase_gradient_rms_radpm',d.phase_gradient_rms_radpm, ...
        'phase_gradient_max_abs_radpm',d.phase_gradient_max_abs_radpm, ...
        'PE_stationary_phase_RMS_rad',d.pe_stationary_phase_rms_rad, ...
        'BH_stationary_phase_RMS_rad',d.bh_stationary_phase_rms_rad, ...
        'two_k_vs_angle_phase_RMS_rad',d.two_k_vs_angle_phase_RMS_rad, ...
        'stationarity_residual_max',g.max_stationarity_residual_m99, ...
        'specular_direction_error_max',g.max_specular_direction_error_m99, ...
        'guards_passed',c.checks.all);
end
case_table=struct2table(rows);
checks=struct('stage4_gate',s4.passed,'representatives_found',numel(cases)==size(spec,1), ...
    'all_case_guards',all(case_table.guards_passed), ...
    'classifications_complete',all(ismember(case_table.classification, ...
    ["global/coherent phase dominated","spatial distortion"])));
checks.all=all(structfun(@(v)logical(v),checks));
out_dir=fullfile(base,cfg.stage5_output_subdir);if ~exist(out_dir,'dir'),mkdir(out_dir);end
validation=struct('schema_version','1.0.0','stage','stage5_phase_attribution', ...
    'root',root,'config',cfg,'representative_rule', ...
    'first non-I height, maximum sampled height, maximum sampled curvature', ...
    'cases',{cases},'case_table',case_table,'checks',checks,'passed',checks.all, ...
    'solver_calls',0,'production_physics_modified',false, ...
    'stationary_geometry_tolerance',cfg.stage5_geometry_tolerance);
mat_file=fullfile(out_dir,'stage5_validation.mat');csv_file=fullfile(out_dir,'stage5_attribution.csv');
report_file=fullfile(out_dir,'stage5_report.md');save(mat_file,'validation','-v7');
writetable(case_table,csv_file);local_stage5_write_report(report_file,validation);
validation.files=struct('mat',mat_file,'csv',csv_file,'report',report_file);result=validation;
if cfg.fail_on_check&&~result.passed,error('Stage 5 failed; see %s.',report_file);end
end

function c=local_stage5_find_case(s2,s3,A,K)
c=[];
for v=[s2.cases,s3.cases]
    q=v{1};
    if abs(q.A_m-A)<1e-12&&abs(q.K_radpm-K)<1e-12,c=q;return,end
end
error('Cannot find saved case A=%.9g, K=%.9g.',A,K);
end

function tf=local_stage5_case_guards(c)
if isfield(c.checks,'field_finite')
    tf=c.checks.geometry&&c.checks.field_finite&&c.checks.diagnostics_finite&&c.checks.metrics_finite;
else
    tf=c.checks.geometry&&c.checks.finite&&c.checks.metrics_finite;
end
end

function label=local_stage5_classification(m)
if m.rho_shape>=.995&&m.aligned_l2_m99<=.5*m.l2_m99
    label="global/coherent phase dominated";
else
    label="spatial distortion";
end
end

function g=local_stage5_stationary_geometry(A,K,x,cfg,fp)
x=x(:).';sgrid=linspace(cfg.wall_support_m(1),cfg.wall_support_m(2),cfg.wall_profile_count);
eta_grid=A*cos(K*sgrid);n=numel(x);sp=zeros(1,n);eta=zeros(1,n);L=zeros(1,n);
res=zeros(1,n);specerr=zeros(1,n);ui=zeros(n,2);uo=zeros(n,2);normal=zeros(n,2);
opts=optimset('TolX',1e-12,'Display','off');
for ii=1:n
    coarse=hypot(sgrid,cfg.z_tx_m-eta_grid)+hypot(x(ii)-sgrid,cfg.z_rx_m-eta_grid);
    [~,jj]=min(coarse);lo=sgrid(max(1,jj-1));hi=sgrid(min(numel(sgrid),jj+1));
    if lo==hi
        ss=lo;
    else
        ss=fminbnd(@(q)local_stage5_path_length(q,x(ii),A,K,cfg),lo,hi,opts);
    end
    ee=A*cos(K*ss);ep=-A*K*sin(K*ss);
    l1=hypot(ss,cfg.z_tx_m-ee);l2=hypot(x(ii)-ss,cfg.z_rx_m-ee);
    uin=[ss/l1,(ee-cfg.z_tx_m)/l1];uout=[(x(ii)-ss)/l2,(cfg.z_rx_m-ee)/l2];
    nn=[-ep,1]/hypot(ep,1);uref=uin-2*dot(uin,nn)*nn;
    sp(ii)=ss;eta(ii)=ee;L(ii)=l1+l2;ui(ii,:)=uin;uo(ii,:)=uout;normal(ii,:)=nn;
    res(ii)=abs((ss+(ee-cfg.z_tx_m)*ep)/l1+ ...
        (ss-x(ii)+(ee-cfg.z_rx_m)*ep)/l2);
    specerr(ii)=norm(uref-uout);
end
flat_length=hypot(x,cfg.z_tx_m+cfg.z_rx_m);k0=2*pi*cfg.frequency_hz/cfg.c0_mps;
cos_i=abs(ui(:,2)).';cos_o=abs(uo(:,2)).';
g=struct('receiver_x_m',x,'specular_s_m',sp,'eta_m',eta,'path_length_m',L, ...
    'flat_path_length_m',flat_length,'incident_direction',ui,'outgoing_direction',uo, ...
    'surface_normal',normal,'stationarity_residual',res, ...
    'specular_direction_error',specerr,'max_stationarity_residual',max(res), ...
    'max_specular_direction_error',max(specerr), ...
    'max_stationarity_residual_m99',max(res(fp.m99.mask)), ...
    'max_specular_direction_error_m99',max(specerr(fp.m99.mask)), ...
    'phase_stationary_rad',-k0*(L-flat_length), ...
    'phase_2k_eta_rad',2*k0*eta, ...
    'phase_angle_aware_rad',k0*(cos_i+cos_o).*eta, ...
    'finite',all(isfinite([sp,eta,L,res,specerr,ui(:).',uo(:).',normal(:).'])));
end

function L=local_stage5_path_length(s,x,A,K,cfg)
eta=A*cos(K*s);L=hypot(s,cfg.z_tx_m-eta)+hypot(x-s,cfg.z_rx_m-eta);
end

function d=local_stage5_phase_diagnostics(c,g,fp,cfg)
m=fp.m99.mask;w=fp.energy_weights(m);w=w/max(sum(w),realmin);x=fp.x_m(m);
pe=c.G_PE(m);bh=c.G_BH_comparison(m);pd=unwrap(angle(pe.*conj(bh)));
grad=gradient(pd,x);sp=g.phase_stationary_rad(m);p2=g.phase_2k_eta_rad(m);pa=g.phase_angle_aware_rad(m);
peerr=angle(pe.*exp(-1i*sp));bherr=angle(bh.*exp(-1i*sp));proxy=angle(exp(1i*(p2-pa)));
d=struct('phase_difference_rad',angle(pe.*conj(bh)), ...
    'phase_gradient_radpm',grad,'phase_gradient_rms_radpm',sqrt(sum(w.*grad.^2)), ...
    'phase_gradient_max_abs_radpm',max(abs(grad)), ...
    'pe_stationary_phase_error_rad',peerr, ...
    'bh_stationary_phase_error_rad',bherr, ...
    'pe_stationary_phase_rms_rad',sqrt(sum(w.*peerr.^2)), ...
    'bh_stationary_phase_rms_rad',sqrt(sum(w.*bherr.^2)), ...
    'two_k_vs_angle_phase_RMS_rad',sqrt(sum(w.*proxy.^2)), ...
    'phase_floor_db',cfg.phase_floor_db, ...
    'finite',all(isfinite([pd,grad,sp,p2,pa,peerr,bherr,proxy])));
end

function local_stage5_write_report(path,v)
fid=fopen(path,'w','n','UTF-8');if fid<0,error('Cannot write %s.',path);end
cl=onCleanup(@()fclose(fid));
fprintf(fid,'# Stage 5 phase discrepancy attribution\n\n状态：**%s**；solver calls: `0`。\n\n',ternary(v.passed,'PASS','FAIL'));
fprintf(fid,'代表点规则：%s。分类严格使用 Goal 的 rho_shape 与 E_aligned/E_G 门限。\n\n',v.representative_rule);
fprintf(fid,'| A | K | region | E_G | E_aligned | reduction | phi0 | rho_shape | phase RMS | TL RMS | phase-gradient RMS/max | PE/BH vs stationary RMS | 2k eta vs angle RMS | classification | guards |\n');
fprintf(fid,'|---:|---:|:---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---|:---:|\n');
for ii=1:height(v.case_table)
    r=v.case_table(ii,:);
    fprintf(fid,'| %.9g | %.9g | %s | %.9g | %.9g | %.3f%% | %.9g | %.9g | %.9g | %.9g | %.9g/%.9g | %.9g/%.9g | %.9g | %s | %s |\n', ...
        r.A_m,r.K_radpm,char(r.region),r.E_G,r.E_aligned,100*r.alignment_reduction_fraction, ...
        r.phi0_rad,r.rho_shape,r.phase_RMS_rad,r.TL_RMS_dB, ...
        r.phase_gradient_rms_radpm,r.phase_gradient_max_abs_radpm, ...
        r.PE_stationary_phase_RMS_rad,r.BH_stationary_phase_RMS_rad, ...
        r.two_k_vs_angle_phase_RMS_rad,char(r.classification),ternary(r.guards_passed,'PASS','FAIL'));
end
...
fprintf(fid,'The stationary/specular, phase-gradient, 2k eta, and angle-aware phase quantities are diagnostics only; no production PE screen or Bellhop physics is changed.\n');
end

function result=local_stage6(cfg,root)
% Stage 6: one canonical fixed-PM realization at 4 kHz. The canonical
% coefficient bytes are copied into the controlled-comparison result tree
% and verified before either solver is called.
prereq={fullfile(cfg.output_dir,cfg.stage1_output_subdir,'stage1_validation.mat'), ...
    fullfile(cfg.output_dir,cfg.stage5_output_subdir,'stage5_validation.mat'), ...
    fullfile(cfg.output_dir,'stage0','stage0_validation.mat')};
for f=prereq
    if exist(f{1},'file')~=2,error('Stage 6 requires %s.',f{1});end
end
q=load(prereq{1},'validation');s1=q.validation;q=load(prereq{2},'validation');s5=q.validation;
q=load(prereq{3},'validation');s0=q.validation;
if ~s1.passed||~s5.passed||~s0.passed,error('Stage 6 prerequisite did not pass.');end
if cfg.frequency_hz~=4000||cfg.stage6_seed~=260001||~strcmpi(cfg.source_geometry,'X')|| ...
        cfg.stage1_beam_counts(2)~=10001
    error('Stage 6 is frozen to seed 260001, 4 kHz, X source, and 10,001 beams.');
end
source_hash=local_sha256_file(cfg.canonical_coeff_source);
if ~strcmp(source_hash,cfg.canonical_coeff_sha256)
    error('Canonical PM coefficient hash mismatch: %s.',source_hash);
end
out_dir=fullfile(cfg.output_dir,cfg.stage6_output_subdir);if ~exist(out_dir,'dir'),mkdir(out_dir);end
coeff_file=fullfile(out_dir,'fixed_pm_fourier_coefficients.csv');
if exist(coeff_file,'file')~=2
    ok=copyfile(cfg.canonical_coeff_source,coeff_file,'f');
    if ~ok,error('Could not copy canonical PM coefficients into Stage 6.');end
end
copied_hash=local_sha256_file(coeff_file);
if ~strcmp(copied_hash,cfg.canonical_coeff_sha256)
    error('Copied Stage-6 coefficient hash mismatch: %s.',copied_hash);
end
profile=load_fixed_pm_profile_for_pe_bellhop_validation(coeff_file, ...
    struct('seed',260001,'wind_speed_mps',6,'requested_kmax_rad_per_m',.5));
case_dir=fullfile(out_dir,'seed260001_f4000_N4097_B10001');
case_file=fullfile(case_dir,'stage6_case.mat');
if exist(case_file,'file')==2
    q=load(case_file,'case_validation');case_validation=q.case_validation;
else
    x=s0.x_m;evpe=evaluate_fixed_pm_fourier_profile(profile,x);
    pe_cfg=struct('frequency_hz',cfg.frequency_hz,'c0_mps',cfg.c0_mps, ...
        'xw_m',cfg.xw_m,'nx',cfg.nx,'z_tx_m',cfg.z_tx_m,'z_rx_m',cfg.z_rx_m, ...
        'sigma_src_m',cfg.sigma_src_m,'surface_elevation_x_m',evpe.eta_m(:).', ...
        'surface_reflect_coeff',-1,'step_m',cfg.pe_step_m,'x_rx_m',0);
    pe=run_pe_1d_surface_reflection_validation(pe_cfg);
    cfg_case=cfg;cfg_case.reuse_existing=true;
    bh=local_stage6_internal(cfg_case,x,profile,cfg.wall_profile_count, ...
        'fixed_pm_seed260001_f4000_N4097_B10001');
    bh.field=local_stage0_convert(bh.pressure_raw,cfg.phase_sign);
    m99=s0.footprint.m99.mask;Gpe=zeros(size(s1.flat_pe));Gbh_raw=zeros(size(s1.flat_bellhop));
    Gpe(m99)=pe.reflected_field(m99)./s1.flat_pe(m99);
    Gbh_raw(m99)=bh.field(m99)./s1.flat_bellhop(m99);Gbh_cmp=conj(Gbh_raw);
    metrics=local_stage1_ratio_metrics(Gpe,Gbh_cmp,s0.footprint);
    stats=local_stage6_profile_stats(profile,cfg.wall_support_m,cfg.wall_profile_count,cfg);
    geometry=local_stage6_geometry(bh.diagnostics,cfg);
    [~,axis_index]=min(abs(x));delta_axis=Gpe(axis_index)/Gbh_cmp(axis_index);
    axis=struct('index',axis_index,'G_PE',Gpe(axis_index),'G_BH_comparison',Gbh_cmp(axis_index), ...
        'delta_tl_db',20*log10(abs(delta_axis)),'delta_phase_rad',angle(delta_axis), ...
        'complex_relative_error',abs(Gpe(axis_index)-Gbh_cmp(axis_index))/max(abs(Gbh_cmp(axis_index)),realmin), ...
        'historical_tl_db',.310371,'historical_phase_rad',-2.248201, ...
        'historical_note','sanity check only; not a hard gate');
    checks=struct('coefficient_hash',strcmp(profile.coeff_file_sha256,cfg.canonical_coeff_sha256), ...
        'geometry',geometry.all,'field_finite',all(isfinite([Gpe(:);Gbh_raw(:);Gbh_cmp(:)])), ...
        'metrics_finite',local_metrics_finite(metrics),'source_X',strcmpi(cfg.source_geometry,'X'), ...
        'beam_count_10001',cfg.stage1_beam_counts(2)==10001);
    checks.all=all(structfun(@(v)logical(v),checks));
    case_validation=struct('schema_version','1.0.0','stage','stage6_fixed_pm_case', ...
        'seed',260001,'frequency_hz',4000,'coefficient_file',coeff_file, ...
        'coefficient_sha256',copied_hash,'profile',profile,'profile_stats',stats, ...
        'pe',pe,'G_PE',Gpe,'G_BH_raw',Gbh_raw,'G_BH_comparison',Gbh_cmp, ...
        'metrics',metrics,'axis_sanity',axis,'geometry',geometry, ...
        'beam_diagnostics',local_stage3_select_diagnostics(bh.diagnostics), ...
        'bellhop_files',bh.files,'checks',checks);
    if ~exist(case_dir,'dir'),mkdir(case_dir);end
    save(case_file,'case_validation','-v7.3');
end
% Recompute comparison metrics from the saved fields so cached cases inherit
% later reporting-only schema additions without rerunning either solver.
saved_diagnostics=local_internal_wall_diagnostics(case_validation.bellhop_files.diagnostics);
case_validation.geometry=local_stage6_geometry(saved_diagnostics,cfg);
case_validation.checks.geometry=case_validation.geometry.all;
case_validation.metrics=local_stage1_ratio_metrics(case_validation.G_PE, ...
    case_validation.G_BH_comparison,s0.footprint);
case_validation.checks.metrics_finite=local_metrics_finite(case_validation.metrics);
if isfield(case_validation.checks,'all'),case_validation.checks=rmfield(case_validation.checks,'all');end
case_validation.checks.all=all(structfun(@(v)logical(v),case_validation.checks));
save(case_file,'case_validation','-v7.3');
region=local_stage4_region(case_validation.metrics,s1.floor);
checks=case_validation.checks;if isfield(checks,'all'),checks=rmfield(checks,'all');end
checks.stage5_gate=s5.passed;checks.all=all(structfun(@(v)logical(v),checks));
validation=struct('schema_version','1.0.0','stage','stage6_fixed_pm', ...
    'root',root,'config',cfg,'case',case_validation,'metrics',case_validation.metrics, ...
    'profile_stats',case_validation.profile_stats,'axis_sanity',case_validation.axis_sanity, ...
    'validity_region',region,'checks',checks,'passed',checks.all, ...
    'solver_calls_allowed',true,'monte_carlo',false);
mat_file=fullfile(out_dir,'stage6_validation.mat');csv_file=fullfile(out_dir,'stage6_metrics.csv');
report_file=fullfile(out_dir,'stage6_report.md');save(mat_file,'validation','-v7.3');
m=validation.metrics;s=validation.profile_stats;a=validation.axis_sanity;
tab=table(260001,4000,m.l2_m99,m.tl_rms_m99,m.phase_rms_m99,m.global_phase_rad, ...
    m.rho_raw,m.rho_shape,m.aligned_l2_m99,s.rms_height_m,s.max_abs_height_m, ...
    s.rms_slope,s.max_abs_slope,s.rms_curvature_per_m,s.max_abs_curvature_per_m, ...
    s.two_k_sigma_eta,a.delta_tl_db,a.delta_phase_rad,region,checks.all, ...
    'VariableNames',{'seed','frequency_hz','E_G','TL_RMS_dB','phase_RMS_rad','phi0_rad', ...
    'rho_raw','rho_shape','E_aligned','rms_height_m','max_abs_height_m','rms_slope', ...
    'max_abs_slope','rms_curvature_per_m','max_abs_curvature_per_m','two_k_sigma_eta', ...
    'axis_delta_TL_dB','axis_delta_phase_rad','region','guards_passed'});
writetable(tab,csv_file);local_stage6_write_report(report_file,validation);
validation.files=struct('coefficient_copy',coeff_file,'case',case_file,'mat',mat_file, ...
    'csv',csv_file,'report',report_file);result=validation;
if cfg.fail_on_check&&~result.passed,error('Stage 6 failed; see %s.',report_file);end
end

function run=local_stage6_internal(cfg,x,profile,N,tag)
[zsort,ord]=sort(-x(:).','ascend');invord=zeros(size(ord));invord(ord)=1:numel(ord);
s=linspace(cfg.wall_support_m(1),cfg.wall_support_m(2),N).';ev=evaluate_fixed_pm_fourier_profile(profile,s);
c=local_stage0_env_cfg(cfg,cfg.parametric_validation_exe,10001,zsort,tag);
c.wall_r0_m=cfg.wall_r0_m;c.wall_seed=260001;c.wall_profile_r_m=cfg.wall_r0_m-ev.eta_m(:).';
c.wall_profile_z_m=s(:).';c.reuse_existing=true;
raw=run_bellhop_internal_pm_wall_poc_vertical(c);run=raw;run.pressure_raw=zeros(1,numel(x));
for ii=1:numel(x)
...
function s=local_stage6_profile_stats(profile,support,N,cfg)
x=linspace(support(1),support(2),N).';e=evaluate_fixed_pm_fourier_profile(profile,x);
kappa=e.eta_second_m_per_m2./(1+e.eta_prime_m_per_m.^2).^(3/2);
e0=evaluate_fixed_pm_fourier_profile(profile,0);k0=2*pi*cfg.frequency_hz/cfg.c0_mps;
s=struct('sample_count',N,'support_m',support,'rms_height_m',sqrt(trapz(x,e.eta_m.^2)/(x(end)-x(1))), ...
    'max_abs_height_m',max(abs(e.eta_m)),'rms_slope',sqrt(trapz(x,e.eta_prime_m_per_m.^2)/(x(end)-x(1))), ...
    'max_abs_slope',max(abs(e.eta_prime_m_per_m)),'rms_curvature_per_m',sqrt(trapz(x,kappa.^2)/(x(end)-x(1))), ...
    'max_abs_curvature_per_m',max(abs(kappa)),'minimum_radius_m',1/max(abs(kappa)), ...
    'two_k_sigma_eta',2*k0*sqrt(trapz(x,e.eta_m.^2)/(x(end)-x(1))), ...
    'eta_at_zero_m',e0.eta_m,'realized_kmax_radpm',profile.realized_kmax_rad_per_m);
end

function g=local_stage6_geometry(d,cfg)
[~,ic]=min(abs(d.alpha_deg));mu=abs(d.inc_ur.*d.wall_n_r+d.inc_uz.*d.wall_n_z);
g=struct('wall_residual_max_m',max(abs(d.wall_residual)), ...
    ...
    'min_post_dr_m',min(d.min_post_dr),'min_abs_incidence',min(mu), ...
    'grazing_fraction',mean(mu<.1),'hit_kappa_max_abs_per_m',max(abs(d.kappa)), ...
    'center_tau_s',d.tau_receiver_real(ic), ...
    'tau_min_s',min(d.tau_receiver_real),'tau_max_s',max(d.tau_receiver_real), ...
    'historical_center_tau_error_s',abs(d.tau_receiver_real(ic)-0.0683459103843));
g.all=all(isfinite(d{:,:}),'all')&&g.wall_residual_max_m<=1e-9&& ...
    g.phase_jump_error_max_rad<=1e-10&&g.p_rotation_error_max<=1e-12&& ...
    g.q_rotation_error_max<=1e-12&&g.min_post_dr_m>0&&g.grazing_fraction==0&& ...
    isfinite(g.center_tau_s)&&cfg.mapped_receiver_range_m==103;
end

function local_stage6_write_report(path,v)
fid=fopen(path,'w','n','UTF-8');if fid<0,error('Cannot write %s.',path);end
cl=onCleanup(@()fclose(fid));m=v.metrics;s=v.profile_stats;a=v.axis_sanity;g=v.case.geometry;
fprintf(fid,'# Stage 6 canonical fixed-PM comparison\n\n状态：**%s**；validity region: **%s**。\n\n',ternary(v.passed,'PASS','FAIL'),char(v.validity_region));
fprintf(fid,'- seed 260001, 4 kHz, X/C, 10,001 beams, N=4097.\n- coefficient SHA-256 `%s`.\n',v.case.coefficient_sha256);
fprintf(fid,'- same Stage-0 flat denominator, receiver line, M95/M99 masks, and official comparison convention.\n\n');
fprintf(fid,'| E_G | TL RMS (dB) | phase RMS (rad) | phi0 | rho_raw | rho_shape | E_aligned |\n|---:|---:|---:|---:|---:|---:|---:|\n');
fprintf(fid,'| %.9g | %.9g | %.9g | %.9g | %.9g | %.9g | %.9g |\n\n',m.l2_m99,m.tl_rms_m99,m.phase_rms_m99,m.global_phase_rad,m.rho_raw,m.rho_shape,m.aligned_l2_m99);
fprintf(fid,'Profile: RMS/max height %.9g/%.9g m; RMS/max slope %.9g/%.9g; RMS/max curvature %.9g/%.9g 1/m; minimum radius %.9g m; 2k sigma_eta %.9g.\n\n', ...
    s.rms_height_m,s.max_abs_height_m,s.rms_slope,s.max_abs_slope,s.rms_curvature_per_m,s.max_abs_curvature_per_m,s.minimum_radius_m,s.two_k_sigma_eta);
fprintf(fid,'Bellhop: wall residual %.3g m; min incidence %.9g; grazing fraction %.9g; hit max |kappa| %.9g 1/m; min post dr %.9g m; phase error %.3g rad; p/q rotation %.3g/%.3g; central/tau range %.12g / [%.12g,%.12g] s.\n\n', ...
    g.wall_residual_max_m,g.min_abs_incidence,g.grazing_fraction,g.hit_kappa_max_abs_per_m,g.min_post_dr_m,g.phase_jump_error_max_rad,g.p_rotation_error_max,g.q_rotation_error_max,g.center_tau_s,g.tau_min_s,g.tau_max_s);
fprintf(fid,'Axis sanity: Delta TL %.9g dB and Delta phase %.9g rad; historical X values %.6g dB / %.6g rad are diagnostic only.\n\n',a.delta_tl_db,a.delta_phase_rad,a.historical_tl_db,a.historical_phase_rad);
fprintf(fid,'Cross-model discrepancy is not a Stage-6 hard gate. No amplitude/phase fit, PM smoothing, source refit, or core-physics change is used.\n');
end

function result=local_stage7(cfg,root)
% Stage 7: interpret the existing M=50 ensemble without running, relabeling,
% or correcting any seed. Its historical point-source R provenance remains
% explicit and is not mixed with the new X-source controlled comparison.
s4f=fullfile(cfg.output_dir,cfg.stage4_output_subdir,'stage4_validation.mat');
s5f=fullfile(cfg.output_dir,cfg.stage5_output_subdir,'stage5_validation.mat');
s6f=fullfile(cfg.output_dir,cfg.stage6_output_subdir,'stage6_validation.mat');
for f={s4f,s5f,s6f}
    if exist(f{1},'file')~=2,error('Stage 7 requires %s.',f{1});end
end
q=load(s4f,'validation');s4=q.validation;q=load(s5f,'validation');s5=q.validation;
q=load(s6f,'validation');s6=q.validation;
if ~s4.passed||~s5.passed||~s6.passed,error('Stage 7 requires PASS results from Stages 4--6.');end
per_seed_file=fullfile(cfg.stage7_historical_dir,'per_seed_results.csv');
stats_file=fullfile(cfg.stage7_historical_dir,'ensemble_statistics.csv');
result_file=fullfile(cfg.stage7_historical_dir,'result.mat');
for f={per_seed_file,stats_file,result_file,cfg.stage7_historical_report}
    if exist(f{1},'file')~=2,error('Missing authoritative M=50 artifact: %s.',f{1});end
end
notice=fileread(cfg.stage7_historical_report);
provenance_ok=contains(notice,'historical Bellhop point-source `R` convention');
rows=readtable(per_seed_file);published=readtable(stats_file);
required={'seed','delta_tl_db','delta_phase_rad','G_pe_power','G_bellhop_power', ...
    'wall_residual_max_m','grazing_fraction','wall_hit_count','expected_beam_count', ...
    'failed_ray_count','rejected_ray_count','phase_jump_error_rad','q_reflect_error', ...
    'p_rotation_error','q_rotation_error','min_post_range_increment_m','all_post_range_positive'};
if ~all(ismember(required,rows.Properties.VariableNames))
    error('Authoritative M=50 CSV is missing required columns.');
end
z=mean(exp(1i*rows.delta_phase_rad));
summary=struct('sample_count',height(rows),'seed_min',min(rows.seed),'seed_max',max(rows.seed), ...
    'mean_delta_tl_db',mean(rows.delta_tl_db),'std_delta_tl_db',std(rows.delta_tl_db), ...
    'mean_pe_power',mean(rows.G_pe_power),'mean_bh_power',mean(rows.G_bellhop_power), ...
    'mean_power_difference',mean(rows.G_pe_power-rows.G_bellhop_power), ...
    'circular_mean_phase_rad',angle(z), ...
    'circular_std_phase_rad',sqrt(max(0,-2*log(max(abs(z),realmin)))), ...
    'phase_resultant_length',abs(z));
published_match=height(published)==1&&published.n==50&& ...
    abs(summary.mean_delta_tl_db-published.delta_tl_mean_db)<=1e-12&& ...
    abs(summary.mean_pe_power-published.pe_power_mean)<=1e-12&& ...
    abs(summary.mean_bh_power-published.bh_power_mean)<=1e-12&& ...
    abs(angle(exp(1i*(summary.circular_mean_phase_rad-published.delta_phase_circular_mean_rad))))<=1e-12;
guard=all(isfinite([rows.delta_tl_db;rows.delta_phase_rad;rows.G_pe_power;rows.G_bellhop_power]))&& ...
    all(rows.wall_hit_count==rows.expected_beam_count)&&all(rows.failed_ray_count==0)&& ...
    all(rows.rejected_ray_count==0)&&all(rows.grazing_fraction==0)&& ...
    all(rows.wall_residual_max_m<=1e-9)&&all(rows.phase_jump_error_rad<=1e-10)&& ...
    all(rows.q_reflect_error<=1e-12)&&all(rows.p_rotation_error<=1e-12)&& ...
    all(rows.q_rotation_error<=1e-12)&&all(rows.min_post_range_increment_m>0)&& ...
    all(rows.all_post_range_positive==1);
checks=struct('stage4_pass',s4.passed,'stage5_pass',s5.passed,'stage6_pass',s6.passed, ...
    'historical_R_provenance_explicit',provenance_ok,'sample_count_50',height(rows)==50, ...
    'seed_range_260001_260050',isequal(rows.seed(:),(260001:260050).'), ...
    'published_statistics_reproduced',published_match,'all_historical_guards',guard);
checks.all=all(structfun(@(v)logical(v),checks));
interpretation=struct( ...
    ...
    'source_limit','No empirical R-to-X correction is applied; M=50 cannot replace the new X-source controlled result.');
out_dir=fullfile(cfg.output_dir,cfg.stage7_output_subdir);if ~exist(out_dir,'dir'),mkdir(out_dir);end
validation=struct('schema_version','1.0.0','stage','stage7_historical_m50_interpretation', ...
    'root',root,'classification','INTERPRETED_WITH_HISTORICAL_R_PROVENANCE', ...
    'historical_source_geometry','R','historical_sample_count',50, ...
    'source_files',struct('per_seed_csv',per_seed_file,'statistics_csv',stats_file, ...
    'result_mat',result_file,'report',cfg.stage7_historical_report), ...
    'source_hashes',struct('per_seed_csv',local_sha256_file(per_seed_file), ...
    'statistics_csv',local_sha256_file(stats_file),'result_mat',local_sha256_file(result_file), ...
    'report',local_sha256_file(cfg.stage7_historical_report)), ...
    'summary',summary,'stage6_fixed_pm_region',s6.validity_region, ...
    'stage6_axis_sanity',s6.axis_sanity,'interpretation',interpretation, ...
    'checks',checks,'passed',checks.all,'solver_calls',0,'seeds_rerun',0, ...
    'empirical_R_to_X_correction',false);
mat_file=fullfile(out_dir,'stage7_validation.mat');csv_file=fullfile(out_dir,'stage7_summary.csv');
report_file=fullfile(out_dir,'stage7_report.md');save(mat_file,'validation','-v7');
tab=struct2table(summary);writetable(tab,csv_file);local_stage7_write_report(report_file,validation);
validation.files=struct('mat',mat_file,'csv',csv_file,'report',report_file);result=validation;
if cfg.fail_on_check&&~result.passed,error('Stage 7 failed; see %s.',report_file);end
end

function local_stage7_write_report(path,v)
fid=fopen(path,'w','n','UTF-8');if fid<0,error('Cannot write %s.',path);end
cl=onCleanup(@()fclose(fid));s=v.summary;
fprintf(fid,'# Stage 7 existing M=50 interpretation\n\n状态：**%s**；solver calls/seeds rerun: `0/0`。\n\n',ternary(v.passed,'PASS','FAIL'));
fprintf(fid,'**Provenance: historical Bellhop point-source R.** These values are not renamed X and receive no empirical R-to-X correction.\n\n');
fprintf(fid,'| M | seed range | mean Delta TL (dB) | std (dB) | PE/BH mean power | mean power difference | circular mean phase (rad) | circular std | R |\n');
fprintf(fid,'|---:|:---:|---:|---:|---:|---:|---:|---:|---:|\n');
fprintf(fid,'| %d | %d--%d | %.9g | %.9g | %.9g / %.9g | %.9g | %.9g | %.9g | %.9g |\n\n', ...
    s.sample_count,s.seed_min,s.seed_max,s.mean_delta_tl_db,s.std_delta_tl_db, ...
    s.mean_pe_power,s.mean_bh_power,s.mean_power_difference,s.circular_mean_phase_rad, ...
    s.circular_std_phase_rad,s.phase_resultant_length);
fprintf(fid,'- Convention: %s\n- Power: %s\n- Phase: %s\n- Validity: %s\n- Source limitation: %s\n\n', ...
    v.interpretation.convention,v.interpretation.power,v.interpretation.phase, ...
    v.interpretation.validity,v.interpretation.source_limit);
