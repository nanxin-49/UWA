...
CONTAINS
  ...
  SUBROUTINE Step2D( ray0, ray2, Topx, Topn, Botx, Botn, WallActive, WallR0, WallSlope, WallHit )
    ...
    REAL (KIND=8 ), INTENT( IN ) :: Topx( 2 ), Topn( 2 ), Botx( 2 ), Botn( 2 ), WallR0, WallSlope
    ...
    CALL ReduceStep2D( ray0%x, urayt0, iSegz0, iSegr0, Topx, Topn, Botx, Botn, &
         WallActive, WallR0, WallSlope, h ) ! reduce h to land on boundary or validation wall
    ...
    CALL ReduceStep2D( ray0%x, urayt1, iSegz0, iSegr0, Topx, Topn, Botx, Botn, &
         WallActive, WallR0, WallSlope, h ) ! reduce h to land on boundary or validation wall
    ...
    ! Validation-only straight internal wall event. The wall participates in
    ! both ReduceStep2D calls above, so a true event ends on
    ! r - WallR0 - WallSlope*z = 0.
    WallHit = WallActive .AND. ABS( ray2%x( 1 ) - WallR0 - WallSlope * ray2%x( 2 ) ) <= &
         MAX( 1.0d-10, 100.0d0 * EPSILON( WallR0 ) * MAX( 1.0d0, ABS( WallR0 ) ) )
  ...
  SUBROUTINE ReduceStep2D( x0, urayt, iSegz0, iSegr0, Topx, Topn, Botx, Botn, &
       WallActive, WallR0, WallSlope, h )
    ...
    ! Validation-only internal straight wall r=WallR0+WallSlope*z. Restrict
    ! the step before integration so the accepted ray point is the analytic
    ! crossing. The signed line function is negative on the water side.
    hWall = HUGE( hWall )
    wallF0  = x0( 1 ) - WallR0 - WallSlope * x0( 2 )
    wallF   = x( 1 )  - WallR0 - WallSlope * x( 2 )
    wallDen = urayt( 1 ) - WallSlope * urayt( 2 )
    wallTol = MAX( 1.0d-12, 100.0d0 * EPSILON( WallR0 ) * MAX( 1.0d0, ABS( WallR0 ) ) )
    IF ( WallActive .AND. wallDen > EPSILON( hWall ) .AND. wallF0 < -wallTol .AND. wallF >= -wallTol ) THEN
       hWall = -wallF0 / wallDen
    END IF

    h = MIN( h, h1, h2, h3, h4, hWall )    ! take limit set by shortest distance to a crossing
    IF ( hWall < HUGE( hWall ) / 2.0d0 .AND. h == hWall ) THEN
       ! A true wall crossing may be arbitrarily close after accumulated
       ! floating-point steps. Preserve that exact event instead of letting
       ! the generic small-step guard push the ray through the wall.
       iSmallStepCtr = 0
    ELSE IF ( h < 1.0d-4 * Beam%deltas ) THEN   ! is it taking an infinitesimal step?
