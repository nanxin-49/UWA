PROGRAM BELLHOP
  ...
  REAL      ( KIND=8 ) :: IWallR0, IWallSlope, IWallReceiverRange
  ...
  CALL ReadInternalTiltedWall( FileRoot )

  CALL OpenOutputFiles( FileRoot, ThreeD )
  OPEN( UNIT = IWallDiagFile, FILE = TRIM( FileRoot ) // '.iwdiag', STATUS = 'REPLACE', ACTION = 'WRITE' )
  WRITE( IWallDiagFile, '(A)' ) '# alpha_deg hit_r hit_z line_residual wall_t_r wall_t_z wall_n_r wall_n_z ' // &
...
SUBROUTINE ReadInternalTiltedWall( FileRootIn )

  CHARACTER (LEN=*), INTENT( IN ) :: FileRootIn
  INTEGER                         :: IWallFile, IOS
  CHARACTER (LEN=256)             :: WallFile

  WallFile = TRIM( FileRootIn ) // '.iw2'
  OPEN( NEWUNIT = IWallFile, FILE = TRIM( WallFile ), STATUS = 'OLD', ACTION = 'READ', IOSTAT = IOS )
  IF ( IOS /= 0 ) CALL ERROUT( 'BELLHOP-IWALL', 'Missing validation-only .iw2 file' )
  READ( IWallFile, *, IOSTAT = IOS ) IWallR0
  IF ( IOS /= 0 ) CALL ERROUT( 'BELLHOP-IWALL', 'Cannot read wall R0 from .iw2' )
  READ( IWallFile, *, IOSTAT = IOS ) IWallSlope
  IF ( IOS /= 0 ) CALL ERROUT( 'BELLHOP-IWALL', 'Cannot read wall slope from .iw2' )
  ...
  IF ( ABS( IWallR0 - 100.0d0 ) > 1.0d-12 ) &
       CALL ERROUT( 'BELLHOP-IWALL', 'Tilted-wall POC requires R0 = 100 m' )
  IF ( ABS( IWallSlope - 0.005d0 ) > 1.0d-12 ) &
       CALL ERROUT( 'BELLHOP-IWALL', 'Tilted-wall POC requires slope a = 0.005' )
  IF ( ABS( IWallReceiverRange - 103.0d0 ) > 1.0d-12 ) &
       CALL ERROUT( 'BELLHOP-IWALL', 'Tilted-wall POC requires mapped receiver range = 103 m' )

  WRITE( PRTFile, * )
  WRITE( PRTFile, * ) 'Validation-only internal tilted straight wall enabled'
  WRITE( PRTFile, * ) 'Wall R0 (m)               = ', IWallR0
  WRITE( PRTFile, * ) 'Wall slope a              = ', IWallSlope
  WRITE( PRTFile, * ) 'Mapped receiver range (m) = ', IWallReceiverRange

END SUBROUTINE ReadInternalTiltedWall
...
SUBROUTINE TraceRay2D( xs, alpha, Amp0 )
  ...
  INTEGER           :: is, is1, jStep, NPost, WallBranchStart ! index for a step along the ray
  ...
  REAL     (KIND=8) :: sss, WallT( 2 ), WallN( 2 ), WallKappa, WallResidual, &
  ...
  WallBranchStart = 0
  WallT            = [ IWallSlope, 1.0d0 ] / SQRT( 1.0d0 + IWallSlope ** 2 )
  WallN            = [ 1.0d0, -IWallSlope ] / SQRT( 1.0d0 + IWallSlope ** 2 )
  WallKappa        = 0.0d0
  WallHS           = Bdry%Top%HS
  WallHS%BC        = 'V'
  ...
  Stepping: DO istep = 1, MaxN - 1
     ...
     CALL Step2D( ray2D( is ), ray2D( is1 ),  &
          ...
          Bot( IsegBot )%x, Bot( IsegBot )%n,  &
           WallActive, IWallR0, IWallSlope, WallHit )
     ...
     IF ( WallHit ) THEN
        ...
        WallResidual  = IncidentState%x( 1 ) - IWallR0 - IWallSlope * IncidentState%x( 2 )

         ! Reuse the native 2D reflector exactly once. The wall frame
         ! t=(a,1)/sqrt(1+a^2), n=(1,-a)/sqrt(1+a^2) has the same sense as
         ! a Bellhop TOP boundary.
  ...
  ExpectedT = [ IWallSlope, 1.0d0 ] / SQRT( 1.0d0 + IWallSlope ** 2 )
  ExpectedN = [ 1.0d0, -IWallSlope ] / SQRT( 1.0d0 + IWallSlope ** 2 )
