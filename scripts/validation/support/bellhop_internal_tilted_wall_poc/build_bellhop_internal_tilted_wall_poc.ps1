...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $projectRoot 'results\validation\bellhop_internal_tilted_wall_poc'
}
...
$validationExe = Join-Path $binDir 'bellhop_iwall_tilted_2020.exe'
...
$manifest = [ordered]@{
    purpose = 'Bellhop 2020 validation-only tilted straight internal wall POC'
