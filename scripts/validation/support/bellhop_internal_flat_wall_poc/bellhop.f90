PROGRAM BELLHOP
  ...
  REAL      ( KIND=8 ) :: IWallRange, IWallReceiverRange
  ...
  CALL ReadInternalFlatWall( FileRoot )

  CALL OpenOutputFiles( FileRoot, ThreeD )
  OPEN( UNIT = IWallDiagFile, FILE = TRIM( FileRoot ) // '.iwdiag', STATUS = 'REPLACE', ACTION = 'WRITE' )
  WRITE( IWallDiagFile, '(A)' ) '# alpha_deg hit_r hit_z residual inc_ur inc_uz ref_ur ref_uz rot_ur rot_uz ' // &
...
SUBROUTINE ReadInternalFlatWall( FileRootIn )
  ...
  READ( IWallFile, *, IOSTAT = IOS ) IWallRange
  IF ( IOS /= 0 ) CALL ERROUT( 'BELLHOP-IWALL', 'Cannot read wall range from .iw2' )
  ...
  WRITE( PRTFile, * ) 'Wall range (m)            = ', IWallRange
  WRITE( PRTFile, * ) 'Mapped receiver range (m) = ', IWallReceiverRange

END SUBROUTINE ReadInternalFlatWall
...
SUBROUTINE TraceRay2D( xs, alpha, Amp0 )
  ...
  Stepping: DO istep = 1, MaxN - 1
     ...
     IF ( WallHit ) THEN
        ...
        WallResidual  = IncidentState%x( 1 ) - IWallRange

        ! Reuse the native 2D reflector exactly once. The wall frame
        ! t=(0,1), n=(1,0) has the same sense as a Bellhop TOP boundary.
        ...
        ! Fixed proper rotation by pi about (IWallRange,0). This is only a
        ! coordinate remapping; do not call Reflect2D or change beam state.
        ray2D( is + 1 )%x = [ 2.0d0 * IWallRange - ray2D( is + 1 )%x( 1 ), &
                              -ray2D( is + 1 )%x( 2 ) ]
  ...
  WRITE( IWallDiagFile, '( *( ES25.16E3, 1X ) )' ) RadDeg * alpha, &
       ...
       IncidentState%c * IncidentState%t( 1 ), IncidentState%c * IncidentState%t( 2 ), &
       ReflectedState%c * ReflectedState%t( 1 ), ReflectedState%c * ReflectedState%t( 2 ), &
       RotatedState%c * RotatedState%t( 1 ), RotatedState%c * RotatedState%t( 2 ), &
