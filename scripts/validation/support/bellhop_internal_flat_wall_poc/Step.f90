...
CONTAINS
  ...
  SUBROUTINE Step2D( ray0, ray2, Topx, Topn, Botx, Botn, WallActive, WallRange, WallHit )
    ...
    REAL (KIND=8 ), INTENT( IN ) :: Topx( 2 ), Topn( 2 ), Botx( 2 ), Botn( 2 ), WallRange
    ...
    CALL ReduceStep2D( ray0%x, urayt0, iSegz0, iSegr0, Topx, Topn, Botx, Botn, &
         WallActive, WallRange, h ) ! reduce h to land on boundary or validation wall
    ...
    ! Validation-only flat internal wall event. The wall participates in
    ! both ReduceStep2D calls above, so a true event ends at r=WallRange.
    WallHit = WallActive .AND. urayt1( 1 ) > 0.0d0 .AND. &
         ABS( ray2%x( 1 ) - WallRange ) <= &
         MAX( 1.0d-10, 100.0d0 * EPSILON( WallRange ) * MAX( 1.0d0, ABS( WallRange ) ) )
  ...
  SUBROUTINE ReduceStep2D( x0, urayt, iSegz0, iSegr0, Topx, Topn, Botx, Botn, &
    ...
    LOGICAL,         INTENT( IN    ) :: WallActive
    REAL (KIND=8), INTENT( IN    ) :: WallRange
    ...
    ! Validation-only internal flat wall r=WallRange. Restrict the step
    ! before integration so the accepted ray point is the true crossing.
    hWall = HUGE( hWall )
    IF ( WallActive .AND. urayt( 1 ) > EPSILON( hWall ) .AND. &
         x0( 1 ) < WallRange .AND. x( 1 ) >= WallRange ) THEN
       hWall = ( WallRange - x0( 1 ) ) / urayt( 1 )
    END IF
