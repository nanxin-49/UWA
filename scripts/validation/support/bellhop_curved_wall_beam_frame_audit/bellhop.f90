PROGRAM BELLHOP
  ...
  LOGICAL              :: IWallEnabled = .FALSE.
  INTEGER, PARAMETER   :: IWallDiagFile = 98
  INTEGER              :: jj
  CHARACTER ( LEN=2  ) :: AttenUnit
  CHARACTER ( LEN=80 ) :: FileRoot
  REAL      ( KIND=8 ) :: IWallR0, IWallAmplitude, IWallWavenumber, IWallReceiverRange
  REAL      ( KIND=8 ) :: AuditCurrentAlpha = 0.0D0
  ...
  INQUIRE( FILE = TRIM( FileRoot ) // '.iw3', EXIST = IWallEnabled )
  IF ( IWallEnabled ) CALL ReadInternalSinusoidalWall( FileRoot )

  CALL OpenOutputFiles( FileRoot, ThreeD )
  CALL ConfigureBeamFrameAudit( FileRoot )
  IF ( IWallEnabled ) THEN
  ...
  END IF
  CALL BellhopCore
  IF ( IWallEnabled ) CLOSE( IWallDiagFile )
  CALL CloseBeamFrameAudit()

  CONTAINS

! **********************************************************************!

SUBROUTINE ReadInternalSinusoidalWall( FileRootIn )
  ...
  WallFile = TRIM( FileRootIn ) // '.iw3'
  OPEN( NEWUNIT = IWallFile, FILE = TRIM( WallFile ), STATUS = 'OLD', ACTION = 'READ', IOSTAT = IOS )
  IF ( IOS /= 0 ) CALL ERROUT( 'BELLHOP-IWALL', 'Missing validation-only .iw3 file' )
  READ( IWallFile, *, IOSTAT = IOS ) IWallR0
  IF ( IOS /= 0 ) CALL ERROUT( 'BELLHOP-IWALL', 'Cannot read wall R0 from .iw3' )
  READ( IWallFile, *, IOSTAT = IOS ) IWallAmplitude
  IF ( IOS /= 0 ) CALL ERROUT( 'BELLHOP-IWALL', 'Cannot read wall amplitude from .iw3' )
  READ( IWallFile, *, IOSTAT = IOS ) IWallWavenumber
  IF ( IOS /= 0 ) CALL ERROUT( 'BELLHOP-IWALL', 'Cannot read wall wavenumber from .iw3' )
  READ( IWallFile, *, IOSTAT = IOS ) IWallReceiverRange
  IF ( IOS /= 0 ) CALL ERROUT( 'BELLHOP-IWALL', 'Cannot read mapped receiver range from .iw3' )
  READ( IWallFile, *, IOSTAT = IOS ) IWallNProfile
  IF ( IOS /= 0 .OR. IWallNProfile < 3 ) CALL ERROUT( 'BELLHOP-IWALL', 'Invalid profile count in .iw3' )
  ...
  DO ii = 1, IWallNProfile
     ...
     IF ( IOS /= 0 ) CALL ERROUT( 'BELLHOP-IWALL', 'Cannot read profile point from .iw3' )
  END DO
  CLOSE( IWallFile )

  IF ( ABS( IWallR0 - 100.0d0 ) > 1.0d-12 ) &
       CALL ERROUT( 'BELLHOP-IWALL', 'Sinusoidal-wall POC requires R0 = 100 m' )
  IF ( IWallAmplitude <= 0.0d0 .OR. ABS( IWallWavenumber ) <= 0.0d0 ) &
  ...
  IF ( MAXVAL( ABS( IWallRProfile - ( IWallR0 - IWallAmplitude * SIN( IWallWavenumber * IWallZProfile ) ) ) ) > 1.0d-8 ) &
       CALL ERROUT( 'BELLHOP-IWALL', 'Profile samples do not match r=R0-A*sin(K*z)' )
  CALL ConfigureWallProfile( IWallRProfile, IWallZProfile, IWallNProfile )

  WRITE( PRTFile, * )
  WRITE( PRTFile, * ) 'Validation-only internal smooth sinusoidal wall enabled'
  WRITE( PRTFile, * ) 'Wall R0 (m)               = ', IWallR0
  WRITE( PRTFile, * ) 'Wall amplitude A (m)      = ', IWallAmplitude
  WRITE( PRTFile, * ) 'Wall wavenumber K (1/m)   = ', IWallWavenumber
  WRITE( PRTFile, * ) 'Wall profile points       = ', IWallNProfile
  WRITE( PRTFile, * ) 'Mapped receiver range (m) = ', IWallReceiverRange

END SUBROUTINE ReadInternalSinusoidalWall
...
SUBROUTINE TraceRay2D( xs, alpha, Amp0 )
  ...
  BeamFrameAuditCurrentAlpha = alpha
  iSmallStepCtr = 0
  CALL EvaluateSSP( xs, c, cimag, gradc, crr, crz, czz, rho, freq, 'TAB' )
  ray2D( 1 )%c         = c
  ray2D( 1 )%x         = xs
  ...
  Stepping: DO istep = 1, MaxN - 1
     ...
     IF ( WallHit ) THEN
        ...
        CALL WriteAuditRotation( alpha, RotatedState )
  ...
  ! Native audit mode keeps the original full trajectory and only uses the
  ! reflection/influence loggers.  The internal-wall diagnostics below are
  ! meaningful only after the validation wall branch has been transformed.
  IF ( .NOT. IWallEnabled ) RETURN
  ...
  ! Analytic tangent/normal of r(z)=R0-A*sin(K*z), oriented in increasing
  ! range over the validated profile interval. These are diagnostics only;
  ! Reflect2D receives the sampled Bellhop frame above.
  ExpectedT = [ -IWallAmplitude * IWallWavenumber * COS( IWallWavenumber * IncidentState%x( 2 ) ), 1.0d0 ]
  ExpectedT = ExpectedT / NORM2( ExpectedT )
  ExpectedN = [ ExpectedT( 2 ), -ExpectedT( 1 ) ]
  WallTError = SQRT( SUM( ( WallT - ExpectedT ) ** 2 ) )
  WallNError = SQRT( SUM( ( WallN - ExpectedN ) ** 2 ) )
...
SUBROUTINE Reflect2D( is, HS, BotTop, tBdry, nBdry, kappa, RefC, Npts )
  ...
  REAL     (KIND=8) :: RM, RN, RNRaw, RNCurv, RNGrad, RNFinal, Tg, Th, rayt( 2 ), rayn( 2 ), rayt_tilde( 2 ), rayn_tilde( 2 ), cnjump, csjump  ! for curvature change
  ...
  RNRaw = 2 * kappa / c ** 2 / Th    ! boundary curvature correction before TOP sign convention
  RN = RNRaw
  ...
  RNGrad = RM * ( 2 * cnjump - RM * csjump ) / c ** 2
  RN = RN + RNGrad
  ...
  IF ( BeamFrameAuditEnabled .AND. BotTop == 'TOP' ) THEN
     WRITE( AuditReflectUnit, '(A,1X,*(ES25.16E3,1X))' ) TRIM( BeamFrameAuditMode ), RadDeg * BeamFrameAuditCurrentAlpha, &
          ray2D( is )%x( 1 ), ray2D( is )%x( 2 ), rayt( 1 ), rayt( 2 ), rayn( 1 ), rayn( 2 ), &
          tBdry( 1 ), tBdry( 2 ), nBdry( 1 ), nBdry( 2 ), kappa, Tg, Th, RNRaw, RNCurv, RNGrad, RNFinal, RM, &
          ray2D( is )%p( 1 ), ray2D( is )%p( 2 ), ray2D( is )%q( 1 ), ray2D( is )%q( 2 ), &
          ray2D( is1 )%p( 1 ), ray2D( is1 )%p( 2 ), ray2D( is1 )%q( 1 ), ray2D( is1 )%q( 2 ), &
          ray2D( is )%Amp, ray2D( is1 )%Amp, ray2D( is )%Phase, ray2D( is1 )%Phase, &
          REAL( ray2D( is )%tau ), AIMAG( ray2D( is )%tau ), REAL( ray2D( is1 )%tau ), AIMAG( ray2D( is1 )%tau ), &
          ray2D( is1 )%t( 1 ), ray2D( is1 )%t( 2 ), rayn_tilde( 1 ), rayn_tilde( 2 ), &
          rayt( 1 ) * rayn( 2 ) - rayt( 2 ) * rayn( 1 )
  END IF
