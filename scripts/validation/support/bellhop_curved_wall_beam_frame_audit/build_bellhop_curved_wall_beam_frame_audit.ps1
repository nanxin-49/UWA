...
$OutputDir = Join-Path $projectRoot 'results\validation\bellhop_curved_wall_beam_frame_audit'
}
...
Copy-Item -LiteralPath (Join-Path $supportDir 'influence.f90') -Destination (Join-Path $buildRoot 'Bellhop\influence.f90') -Force
...
$validationExe = Join-Path $binDir 'bellhop_curved_wall_audit_2020.exe'
...
$manifest = [ordered]@{
    purpose = 'Bellhop 2020 validation-only curved-wall beam/frame covariance audit'
    ...
    overlay_influence_sha256 = (Get-FileHash -Algorithm SHA256 (Join-Path $supportDir 'influence.f90')).Hash
