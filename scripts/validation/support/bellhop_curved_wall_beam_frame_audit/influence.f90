MODULE Influence
  ...
  INTEGER :: AuditReflectUnit = 91, AuditRotationUnit = 90, AuditContributionUnit = 92
  LOGICAL :: BeamFrameAuditEnabled = .FALSE.
  REAL    (KIND=8) :: BeamFrameAuditReceiverRange = -HUGE( 1.0D0 )
  REAL    (KIND=8) :: BeamFrameAuditReceiverDepth = 0.0D0
  REAL    (KIND=8) :: BeamFrameAuditCurrentAlpha = 0.0D0
  REAL    (KIND=8) :: BeamFrameAuditRayT( 2 ) = 0.0D0, BeamFrameAuditRayN( 2 ) = 0.0D0, BeamFrameAuditRadiusMax = 0.0D0
  CHARACTER (LEN=16) :: BeamFrameAuditMode = 'disabled'

CONTAINS

  SUBROUTINE ConfigureBeamFrameAudit( FileRoot )
    CHARACTER (LEN=*), INTENT( IN ) :: FileRoot
    CHARACTER (LEN=256) :: AuditFile
    INTEGER :: IOS
    LOGICAL :: Exists

    BeamFrameAuditEnabled = .FALSE.
    AuditFile = TRIM( FileRoot ) // '.audit'
    INQUIRE( FILE = TRIM( AuditFile ), EXIST = Exists )
    IF ( .NOT. Exists ) RETURN

    OPEN( UNIT = AuditReflectUnit, FILE = TRIM( FileRoot ) // '.iwaudit_reflect', STATUS = 'REPLACE', ACTION = 'WRITE', IOSTAT = IOS )
    IF ( IOS /= 0 ) STOP 'BELLHOP-AUDIT: cannot open reflection audit file'
    OPEN( UNIT = AuditRotationUnit, FILE = TRIM( FileRoot ) // '.iwaudit_rotation', STATUS = 'REPLACE', ACTION = 'WRITE', IOSTAT = IOS )
    IF ( IOS /= 0 ) STOP 'BELLHOP-AUDIT: cannot open rotation audit file'
    OPEN( UNIT = AuditContributionUnit, FILE = TRIM( FileRoot ) // '.iwaudit_contribution', STATUS = 'REPLACE', ACTION = 'WRITE', IOSTAT = IOS )
    IF ( IOS /= 0 ) STOP 'BELLHOP-AUDIT: cannot open contribution audit file'
    OPEN( UNIT = 89, FILE = TRIM( AuditFile ), STATUS = 'OLD', ACTION = 'READ', IOSTAT = IOS )
    IF ( IOS /= 0 ) STOP 'BELLHOP-AUDIT: cannot read audit sidecar'
    READ( 89, *, IOSTAT = IOS ) BeamFrameAuditMode
    IF ( IOS /= 0 ) STOP 'BELLHOP-AUDIT: cannot read audit mode'
    READ( 89, *, IOSTAT = IOS ) BeamFrameAuditReceiverRange
    IF ( IOS /= 0 ) STOP 'BELLHOP-AUDIT: cannot read audit receiver range'
    READ( 89, *, IOSTAT = IOS ) BeamFrameAuditReceiverDepth
    IF ( IOS /= 0 ) STOP 'BELLHOP-AUDIT: cannot read audit receiver depth'
    CLOSE( 89 )

    WRITE( AuditReflectUnit, '(A)' ) '# mode alpha_deg x_r x_z rayt_r rayt_z rayn_r rayn_z wallt_r wallt_z walln_r walln_z kappa Tg Th RN_raw RN_curv RN_grad RN_final RM p0_1 p0_2 q0_1 q0_2 p1_1 p1_2 q1_1 q1_2 amp0 amp1 phase0 phase1 tau0_r tau0_i tau1_r tau1_i ref_t_r ref_t_z ref_n_r ref_n_z handedness'
    WRITE( AuditRotationUnit, '(A)' ) '# mode alpha_deg x_r x_z tangent_r tangent_z rayn_r rayn_z p1 p2 q1 q2 amp phase tau_r tau_i handedness q_sign'
    WRITE( AuditContributionUnit, '(A)' ) '# mode alpha_deg iS ir iz x0_r x0_z x1_r x1_z rayt_r rayt_z rayn_r rayn_z p0_1 p0_2 q0_1 q0_2 p_interp_1 p_interp_2 q_interp tau_r tau_i n radius q0_ref W amp phase_kmah phase_int beam_amp beam_phase contribution_r contribution_i handedness'
    BeamFrameAuditEnabled = .TRUE.
  END SUBROUTINE ConfigureBeamFrameAudit

  SUBROUTINE CloseBeamFrameAudit()
    IF ( .NOT. BeamFrameAuditEnabled ) RETURN
    CLOSE( AuditReflectUnit )
    CLOSE( AuditRotationUnit )
    CLOSE( AuditContributionUnit )
    BeamFrameAuditEnabled = .FALSE.
  END SUBROUTINE CloseBeamFrameAudit

  SUBROUTINE WriteAuditContribution( RayTIn, RayNIn, RadiusMaxIn )
    REAL (KIND=8), INTENT( IN ) :: RayTIn( 2 ), RayNIn( 2 ), RadiusMaxIn
    COMPLEX (KIND=8) :: Contribution
    REAL (KIND=8) :: Handedness

    IF ( .NOT. BeamFrameAuditEnabled ) RETURN
    Contribution = CMPLX( Amp * EXP( -i * ( omega * delay - phaseInt ) ), KIND=8 )
    Handedness = RayTIn( 1 ) * RayNIn( 2 ) - RayTIn( 2 ) * RayNIn( 1 )
    WRITE( AuditContributionUnit, '(A,1X,*(ES25.16E3,1X))' ) TRIM( BeamFrameAuditMode ), SrcDeclAngle, REAL( iS, KIND=8 ), REAL( ir, KIND=8 ), REAL( iz, KIND=8 ), &
         ray2D( iS - 1 )%x( 1 ), ray2D( iS - 1 )%x( 2 ), ray2D( iS )%x( 1 ), ray2D( iS )%x( 2 ), RayTIn( 1 ), RayTIn( 2 ), RayNIn( 1 ), RayNIn( 2 ), &
         ray2D( iS - 1 )%p( 1 ), ray2D( iS - 1 )%p( 2 ), ray2D( iS - 1 )%q( 1 ), ray2D( iS - 1 )%q( 2 ), &
         ray2D( iS - 1 )%p( 1 ) + s * ( ray2D( iS )%p( 1 ) - ray2D( iS - 1 )%p( 1 ) ), &
         ray2D( iS - 1 )%p( 2 ) + s * ( ray2D( iS )%p( 2 ) - ray2D( iS - 1 )%p( 2 ) ), q, REAL( delay ), AIMAG( delay ), n, RadiusMaxIn, q0, W, Amp, phase, phaseInt, ray2D( iS )%Amp, ray2D( iS )%Phase, &
         REAL( Contribution ), AIMAG( Contribution ), Handedness
  END SUBROUTINE WriteAuditContribution

  SUBROUTINE WriteAuditRotation( Alpha, State )
    REAL (KIND=8), INTENT( IN ) :: Alpha
    TYPE( ray2DPt ), INTENT( IN ) :: State
    REAL (KIND=8) :: RayT( 2 ), RayN( 2 ), Handedness, QSign

    IF ( .NOT. BeamFrameAuditEnabled ) RETURN
    RayT = State%c * State%t
    RayN = [ -RayT( 2 ), RayT( 1 ) ]
    Handedness = RayT( 1 ) * RayN( 2 ) - RayT( 2 ) * RayN( 1 )
    QSign = SIGN( 1.0D0, State%q( 1 ) )
    WRITE( AuditRotationUnit, '(A,1X,*(ES25.16E3,1X))' ) TRIM( BeamFrameAuditMode ), RadDeg * Alpha, State%x( 1 ), State%x( 2 ), &
         State%t( 1 ), State%t( 2 ), RayN( 1 ), RayN( 2 ), State%p( 1 ), State%p( 2 ), State%q( 1 ), State%q( 2 ), &
         State%Amp, State%Phase, REAL( State%tau ), AIMAG( State%tau ), Handedness, QSign
  END SUBROUTINE WriteAuditRotation
...
!!! need to add logic related to NRz_per_range
  ...
  SUBROUTINE InfluenceGeoHatCart( U, alpha, Dalpha )
    ...
    Stepping: DO iS = 2, Beam%Nsteps
       ...
       BeamFrameAuditRayT = rayt
       BeamFrameAuditRayN = rayn
       BeamFrameAuditRadiusMax = RadiusMax
  ...
  SUBROUTINE ApplyContribution( U )
    ...
    IF ( BeamFrameAuditEnabled .AND. Beam%RunType( 1 : 1 ) == 'C' .AND. &
         ir >= 1 .AND. ir <= Pos%NRr .AND. iz >= 1 .AND. iz <= NRz_per_range .AND. &
         ray2D( iS )%NumTopBnc == 1 .AND. ray2D( iS )%NumBotBnc == 0 .AND. &
         ABS( Pos%Rr( ir ) - BeamFrameAuditReceiverRange ) <= 1.0D-8 .AND. &
         ABS( Pos%Rz( iz ) - BeamFrameAuditReceiverDepth ) <= 1.0D-8 ) THEN
       CALL WriteAuditContribution( BeamFrameAuditRayT, BeamFrameAuditRayN, BeamFrameAuditRadiusMax )
    END IF
