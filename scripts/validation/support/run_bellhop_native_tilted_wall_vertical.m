function result = run_bellhop_native_tilted_wall_vertical(cfg)
%RUN_BELLHOP_NATIVE_TILTED_WALL_VERTICAL Run an equivalent native ATI case.
% This helper is validation-only: the physical wall is represented by the
% original Bellhop top-boundary machinery, with a linear ATI profile.
arguments
    cfg (1,1) struct
end
required={'bellhop_exe','case_root','run_type','frequency_hz','c0_mps', ...
    ...
    'source_pattern_level_db','wall_r0_m','wall_slope'};
...
% A linear ATI profile is exact for r=R0+a*z.  The points bracket the
% receiver and wall region; Bellhop extends the first/last segment as usual.
z_ati=[-1000 0 1000];
r_ati=cfg.wall_r0_m+cfg.wall_slope*z_ati;
...
cleanup=onCleanup(@()fclose(fid));
fprintf(fid,'''Native tilted ATI wall comparison''\n%.17g\n1\n''CVW *''\n',cfg.frequency_hz);
