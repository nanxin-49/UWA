...
switch lower(char(stage))
    ...
    case {'strong','strongh'}
        assert(isscalar(sigma_m)&&ismember(sigma_m,cfg.strong_sigma_m), ...
            'strong requires one configured sigma.');
        result=local_case(cfg,root,sigma_m,cfg.strong_A_m,cfg.strong_K_radpm,'strongH');
    case 'aggregate'
...
cfg=struct('frequency_hz',4000,'c0_mps',1500,'z_tx_m',100,'z_rx_m',3, ...
    ...
    'strong_sigma_m',[0.3 2],'strong_A_m',0.20,'strong_K_radpm',0.10, ...
...
elseif abs(A-.20)<=eps&&abs(K-.10)<=eps
    path=fullfile(root,'results','validation','pe_bellhop_helmholtz_bie_reference', ...
...
strong=cell(numel(cfg.strong_sigma_m),1);
for ii=1:numel(strong),strong{ii}=local_load_case(cfg,'strongH',cfg.strong_sigma_m(ii));end
low_table=local_table(low,cfg);high_table=local_table(high,cfg);strong_table=local_table(strong,cfg);
optional=[];
...
checks=struct('all_numerical',all(cellfun(@(q)q.passed,[low;high;strong])), ...
...
result=struct('schema_version','1.0.0','stage','strict_normal_sigma_aggregate', ...
    'config',cfg,'lowK',low_table,'highK',high_table,'strongH',strong_table, ...
...
local_report_table(fid,'Strong-height A=0.20 m, K=0.10 rad/m',v.strongH);
...
fprintf(fid,'- Strong-height check (`A=0.20 m, K=0.10 rad/m`): narrowing from `theta95=%.6g` to `%.6g deg` changes the PE Model-0/Model-1 gap from `%.6g` to `%.6g`, while PE--BIE complex errors remain `%.6g` and `%.6g`. This separates the finite-angle effect from the strong-height phase-screen residual.\n\n', ...
    v.strongH.theta95_deg(1),v.strongH.theta95_deg(end),v.strongH.Model0_Model1_E_G(1),v.strongH.Model0_Model1_E_G(end), ...
    v.strongH.Model0_E_G(1),v.strongH.Model0_E_G(end));
