...
if exist(ensemble_file,'file')
    ...
    for ww=1:numel(winds)
        ...
        Hkd_total=d.cache.H_direct_f+d.cache.H_ref_coh_f+Hkd;
        Hj_total=d.cache.H_direct_f+d.cache.H_ref_coh_f+Hj;
        ...
        ensembles.(key)=struct( ...
            'kdomain_pe',local_set('kdomain_pe',U,d.cache.f_axis_hz,Hkd_total), ...
            'joint_cached_pe',local_set('joint_cached_pe',U,d.cache.f_axis_hz,Hj_total), ...
            'stats_full',local_set('stats_full',U,m.model.frequency_axis,sf.H_total_f), ...
            'stats_99_9',local_set('stats_99_9',U,m.model.frequency_axis,sl.H_total_f));
    ...
    save(ensemble_file,'ensembles','generation','-v7.3');
end
...
for ww=1:numel(winds)
    ...
    md=load(mf,'model'); p=sample_conditional_channel_rank_pair_vertical(md.model,L_rank_pair,3800000+U*10000,'99.9');
    cfg=comm_cfg; cfg.bootstrap_seed=3900000+U*10000;
    rf=evaluate_mpsk_channel_ensemble_vertical(local_set('paired_full',U,md.model.frequency_axis,p.H_full_f),cfg);
    rl=evaluate_mpsk_channel_ensemble_vertical(local_set('paired_99_9',U,md.model.frequency_axis,p.H_low_f),cfg);
...
for ww=1:numel(winds)
    ...
    for ss=1:numel(source_names)
        ...
        cir=build_physical_cir_vertical(H,ensembles.(key).(name).f_axis_hz,-103/1500,'none',1);
        [~,legacy]=build_communication_taps_vertical(struct('H_f',H,'f_axis_hz',ensembles.(key).(name).f_axis_hz), ...
...
validation=struct('mode',mode,'winds_mps',winds,'channel_count_per_source',L, ...
    ...
    'noise_in_channel',false,'public_defaults_changed',false);
save(fullfile(out_dir,['two_node_communication_validation_' mode '.mat']),'validation','-v7.3');
...
function s=local_set(name,U,f,H)
