...
for ii=1:numel(seeds)
    ...
    kappa=es.eta_second_m_per_m2./(1+es.eta_prime_m_per_m.^2).^(3/2); r=local_empty_row();r.seed=seed;r.coeff_file_sha256=profile.coeff_file_sha256;r.profile_band_match=band_match;r.profile_rms_height_m=sqrt(mean(es.eta_m.^2));r.profile_rms_slope=sqrt(mean(es.eta_prime_m_per_m.^2));r.profile_max_slope=max(abs(es.eta_prime_m_per_m));r.profile_rms_curvature_per_m=sqrt(mean(kappa.^2));r.profile_max_curvature_per_m=max(abs(kappa));r.profile_min_radius_m=1/max(max(abs(kappa)),eps);r.G_pe=Gpe;r.G_bellhop=Gbh;r.delta_tl_db=20*log10(abs(Gpe/Gbh));r.delta_phase_rad=angle(Gpe*conj(Gbh));r.complex_relative_error=abs(Gpe-Gbh)/max(abs(Gbh),realmin);r.wall_residual_max_m=max(abs(d.wall_residual));r.min_mu=min(mu);r.grazing_fraction=mean(mu<cfg.grazing_mu_threshold);r.wall_hit_count=height(d);r.expected_beam_count=cfg.beam_count;r.successful_wall_reflection_fraction=r.wall_hit_count/r.expected_beam_count;r.bellhop_case_source=case_meta.source;r.bellhop_case_fingerprint=case_meta.fingerprint_sha256;r.bellhop_beam_count=bh.config.beam_count;r.phase_jump_error_rad=max(abs(d.phase_delta-pi));r.q_reflect_error=max(abs(d.q_ref_error));r.p_rotation_error=max(d.p_rot_error);r.q_rotation_error=max(d.q_rot_error);r.min_post_range_increment_m=min(d.min_post_dr);r.all_post_range_positive=all(d.min_post_dr>0);r.tau_receiver_s=d.tau_receiver_real(find(abs(d.alpha_deg)==min(abs(d.alpha_deg)),1));rows(ii)=r;
...
c=struct('bellhop_exe',cfg.flat_validation_exe, ...
    'case_root',fullfile(cfg.output_dir,'cases',sprintf('flat_B%d',cfg.beam_count)), ...
    'run_type','C','frequency_hz',cfg.frequency_hz,'c0_mps',cfg.c0_mps, ...
...
c=struct('bellhop_exe',cfg.pm_validation_exe, ...
    'case_root',fullfile(cfg.output_dir,'cases',sprintf('seed_%d_B%d',seed,cfg.beam_count)), ...
    'run_type','C','frequency_hz',cfg.frequency_hz,'c0_mps',cfg.c0_mps, ...
    'source_depth_m',0,'receiver_depths_m',0,'receiver_ranges_m',[102 103], ...
    'beam_count',cfg.beam_count,'angle_limits_deg',cfg.angle_limits_deg, ...
    'domain_half_depth_m',cfg.domain_half_depth_m,'sigma_src_m',cfg.sigma_src_m, ...
...
summary=struct('schema_version','1.0.0','case_kind',kind, ...
    'run_type',upper(char(c.run_type(1))),'frequency_hz',double(c.frequency_hz), ...
...
row=struct('seed',NaN,'coeff_file_sha256','','profile_band_match',false,'profile_rms_height_m',NaN,'profile_rms_slope',NaN,'profile_max_slope',NaN,'profile_rms_curvature_per_m',NaN,'profile_max_curvature_per_m',NaN,'profile_min_radius_m',NaN,'G_pe',complex(NaN),'G_bellhop',complex(NaN),'delta_tl_db',NaN,'delta_phase_rad',NaN,'complex_relative_error',NaN,'wall_residual_max_m',NaN,'min_mu',NaN,'grazing_fraction',NaN,'wall_hit_count',NaN,'expected_beam_count',NaN,'successful_wall_reflection_fraction',NaN,'bellhop_case_source','','bellhop_case_fingerprint','','bellhop_beam_count',NaN,'phase_jump_error_rad',NaN,'q_reflect_error',NaN,'p_rotation_error',NaN,'q_rotation_error',NaN,'min_post_range_increment_m',NaN,'all_post_range_positive',false,'tau_receiver_s',NaN);
end
...
fprintf(fid,'# %s\n\n状态：**%s**\n\n',report_title,a.classification);fprintf(fid,'固定 4 kHz、uniform c=1500 m/s、W=192.1875 m、nx=984、step=0.05 m、Bellhop %d beams、sector ±15°。%s\n\n',a.config.beam_count,mode_text);
