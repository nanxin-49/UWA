...
for ifq = 1:Nf
    ...
    if cfg.enable_surface_reflection
        ...
        if cfg.surface_ssa_component_diagnostics && ...
                strcmp(cfg.surface_boundary_model, 'ssa_stat_kernel') && ...
                isfield(rough_meta_f, 'ssa_stat_kernel_meta') && ...
                rough_meta_f.ssa_stat_kernel_meta.enabled
            R_coh_component = rough_meta_f.ssa_stat_kernel_meta.R_coh;
            psi_coh_f = R_coh_component .* psi_surface_inc;
            psi_sca_f = psi_ref_f - psi_coh_f;
            disabled_slice_request = struct('enabled', false);
            [psi_coh_at_rx, ~, ~] = local_march_field( ...
                psi_coh_f, 0, rx_state_used.z_rx, cfg, x, y, alpha_xy_work, ...
                kappa2_work, k0, f_hz, use_gpu, disabled_slice_request);
            [psi_sca_at_rx, ~, ~] = local_march_field( ...
                psi_sca_f, 0, rx_state_used.z_rx, cfg, x, y, alpha_xy_work, ...
                kappa2_work, k0, f_hz, use_gpu, disabled_slice_request);
            surface_ssa_component_meta = local_record_surface_ssa_component( ...
                surface_ssa_component_meta, ifq, f_hz, R_coh_component, ...
                h_reflect_fi, ...
                psi_coh_at_rx(iy_rx, ix_rx), psi_sca_at_rx(iy_rx, ix_rx), ...
                rough_meta_f.ssa_stat_kernel_meta);
        end
...
function meta = local_disabled_surface_ssa_component_meta(cfg, f_axis)
Nf = numel(f_axis);
meta = struct( ...
    'enabled', false, ...
    'model', 'off', ...
    ...
    'random_scatter_enabled', cfg.surface_ssa_random_scatter, ...
    'description', ['Disabled by default. When enabled for ssa_stat_kernel, ', ...
        'the surface reflected field is decomposed as Psi_coh=R_coh*Psi_inc ', ...
        'and Psi_sca=Psi_ref-Psi_coh, then each component is marched to the receiver.'], ...
    'f_axis_hz', f_axis(:), ...
    'R_coh_f', complex(NaN(Nf, 1), NaN(Nf, 1)), ...
    'h_reflect_coh_f', complex(NaN(Nf, 1), NaN(Nf, 1)), ...
    'h_reflect_sca_f', complex(NaN(Nf, 1), NaN(Nf, 1)), ...
    'h_reflect_total_f', complex(NaN(Nf, 1), NaN(Nf, 1)), ...
    'abs_h_reflect_coh_f', NaN(Nf, 1), ...
    'abs_h_reflect_sca_f', NaN(Nf, 1), ...
    'abs_h_reflect_total_f', NaN(Nf, 1), ...
    'scatter_to_coherent_rx_db_f', NaN(Nf, 1), ...
    'scatter_to_total_rx_db_f', NaN(Nf, 1), ...
    'coherent_to_total_rx_db_f', NaN(Nf, 1), ...
    'component_phase_diff_rad_f', NaN(Nf, 1), ...
    'component_sum_error_abs_f', NaN(Nf, 1), ...
    'component_sum_error_rel_f', NaN(Nf, 1), ...
    'surface_E_inc_f', NaN(Nf, 1), ...
    'surface_E_coh_f', NaN(Nf, 1), ...
    'surface_E_sca_f', NaN(Nf, 1), ...
    'surface_E_sca_limited_f', NaN(Nf, 1), ...
    'surface_E_ref_f', NaN(Nf, 1), ...
    'surface_E_coh_over_E_inc_f', NaN(Nf, 1), ...
    'surface_E_sca_over_E_inc_f', NaN(Nf, 1), ...
    'surface_E_sca_limited_over_E_inc_f', NaN(Nf, 1), ...
    'surface_E_ref_over_E_inc_f', NaN(Nf, 1), ...
    'scatter_energy_fraction_db_f', NaN(Nf, 1), ...
    'energy_conservation_error_f', NaN(Nf, 1), ...
    'energy_limit_applied_f', false(Nf, 1), ...
    'W_eta_variance_rel_error_f', NaN(Nf, 1), ...
    'recorded_frequency_mask', false(Nf, 1));
end

function meta = local_record_surface_ssa_component( ...
    meta, ifq, f_hz, R_coh, h_reflect_total, h_reflect_coh, h_reflect_sca, ssa_meta)
meta.enabled = true;
meta.model = 'ssa_surface_component_diagnostics_v1';
meta.f_axis_hz(ifq) = f_hz;
meta.R_coh_f(ifq) = R_coh;
meta.h_reflect_coh_f(ifq) = h_reflect_coh;
meta.h_reflect_sca_f(ifq) = h_reflect_sca;
meta.h_reflect_total_f(ifq) = h_reflect_total;
meta.abs_h_reflect_coh_f(ifq) = abs(h_reflect_coh);
meta.abs_h_reflect_sca_f(ifq) = abs(h_reflect_sca);
meta.abs_h_reflect_total_f(ifq) = abs(h_reflect_total);
meta.scatter_to_coherent_rx_db_f(ifq) = 20 * log10( ...
    max(abs(h_reflect_sca), eps) / max(abs(h_reflect_coh), eps));
meta.scatter_to_total_rx_db_f(ifq) = 20 * log10( ...
    max(abs(h_reflect_sca), eps) / max(abs(h_reflect_total), eps));
meta.coherent_to_total_rx_db_f(ifq) = 20 * log10( ...
    max(abs(h_reflect_coh), eps) / max(abs(h_reflect_total), eps));
meta.component_phase_diff_rad_f(ifq) = angle(h_reflect_sca) - angle(h_reflect_coh);
component_sum_error = h_reflect_total - h_reflect_coh - h_reflect_sca;
meta.component_sum_error_abs_f(ifq) = abs(component_sum_error);
meta.component_sum_error_rel_f(ifq) = abs(component_sum_error) / max(abs(h_reflect_total), eps);
meta.surface_E_inc_f(ifq) = local_get_meta_scalar(ssa_meta, 'E_inc');
meta.surface_E_coh_f(ifq) = local_get_meta_scalar(ssa_meta, 'E_coh');
meta.surface_E_sca_f(ifq) = local_get_meta_scalar(ssa_meta, 'E_sca');
meta.surface_E_sca_limited_f(ifq) = local_get_meta_scalar(ssa_meta, 'E_sca_limited');
meta.surface_E_ref_f(ifq) = local_get_meta_scalar(ssa_meta, 'E_ref');
meta.surface_E_coh_over_E_inc_f(ifq) = meta.surface_E_coh_f(ifq) / ...
    max(meta.surface_E_inc_f(ifq), eps);
meta.surface_E_sca_over_E_inc_f(ifq) = meta.surface_E_sca_f(ifq) / ...
    max(meta.surface_E_inc_f(ifq), eps);
meta.surface_E_sca_limited_over_E_inc_f(ifq) = meta.surface_E_sca_limited_f(ifq) / ...
    max(meta.surface_E_inc_f(ifq), eps);
meta.surface_E_ref_over_E_inc_f(ifq) = meta.surface_E_ref_f(ifq) / ...
    max(meta.surface_E_inc_f(ifq), eps);
meta.scatter_energy_fraction_db_f(ifq) = 10 * log10( ...
    max(meta.surface_E_sca_limited_f(ifq), eps) / max(meta.surface_E_coh_f(ifq), eps));
meta.energy_conservation_error_f(ifq) = local_get_meta_scalar(ssa_meta, 'energy_conservation_error');
if isfield(ssa_meta, 'energy_limit_applied')
    meta.energy_limit_applied_f(ifq) = logical(ssa_meta.energy_limit_applied);
end
meta.W_eta_variance_rel_error_f(ifq) = local_get_meta_scalar(ssa_meta, 'W_eta_variance_rel_error');
meta.recorded_frequency_mask(ifq) = true;
end

function value = local_get_meta_scalar(meta, field_name)
if isstruct(meta) && isfield(meta, field_name) && isscalar(meta.(field_name))
