function output = vertical_channel_model_impl(paramsV)
%VERTICAL_CHANNEL_MODEL_IMPL Implementation behind the public root wrapper.
...
output.H_f = H_f;
output.H_direct_reduced_f = phase_reference_output.H_direct_reduced_f;
output.H_reflect_reduced_f = phase_reference_output.H_reflect_reduced_f;
output.H_total_reduced_f = phase_reference_output.H_total_reduced_f;
output.H_direct_physical_f = phase_reference_output.H_direct_physical_f;
output.H_reflect_physical_f = phase_reference_output.H_reflect_physical_f;
output.H_physical_f = phase_reference_output.H_physical_f;
output.h_direct_reduced = phase_reference_output.h_direct_reduced;
output.h_reflect_reduced = phase_reference_output.h_reflect_reduced;
output.h_total_reduced = phase_reference_output.h_total_reduced;
output.h_direct_physical = phase_reference_output.h_direct_physical;
output.h_reflect_physical = phase_reference_output.h_reflect_physical;
output.h_total_physical = phase_reference_output.h_total_physical;
output.phase_reference_meta = phase_reference_output.phase_reference_meta;
...
defaults = struct( ...
    ...
    'bubble_wind_speed', [], ...
    'sea_hs_target', 0.5, ...
    'surface_roughness_scale_mode', 'target_hs', ...
    'sea_seed', 12345, ...
    'surface_elevation_override_xy', [], ...
    ...
    'surface_ssa_frequency_correlation_mode', 'independent', ...
    'surface_ssa_frequency_correlation_rho', 0.8, ...
    'surface_ssa_geometry_source_id', '', ...
    'surface_ssa_kz_branch', 'downward_positive_real', ...
    'surface_ssa_conv_padding', 'periodic', ...
    'surface_ssa_component_diagnostics', false, ...
    'surface_kstat_random_scatter', true, ...
    'surface_kstat_seed_offset', 200000, ...
    'surface_kstat_conv_padding', 'periodic', ...
    'surface_kstat_trusted_angle_deg', NaN, ...
...
cfg.surface_wavefield_max_z_samples = local_force_int( ...
    cfg.surface_wavefield_max_z_samples, 'surface_wavefield_max_z_samples');
...
window_limit_m=100;
if cfg.validation_allow_extended_window, window_limit_m=400; end
...
cfg.channel_phase_reference = local_normalize_choice( ...
    cfg.channel_phase_reference,'channel_phase_reference');
if ~ismember(cfg.channel_phase_reference,{'direct_dsp','legacy_reduced'})
    error('channel_phase_reference must be ''direct_dsp'' or ''legacy_reduced''.');
end
...
cfg.surface_roughness_scale_mode = local_normalize_choice( ...
    cfg.surface_roughness_scale_mode, 'surface_roughness_scale_mode');
if ~any(strcmp(cfg.surface_roughness_scale_mode, {'target_hs', 'raw_pm'}))
    error('surface_roughness_scale_mode must be ''target_hs'' or ''raw_pm''.');
end
...
if ~any(strcmp(cfg.surface_boundary_model, {'kirchhoff_spatial', 'kirchhoff_kdomain', 'ssa_stat_kernel', 'kirchhoff_kstat'}))
    error('surface_boundary_model must be ''kirchhoff_spatial'', ''kirchhoff_kdomain'', ''ssa_stat_kernel'', or ''kirchhoff_kstat''.');
end
if ~isempty(cfg.surface_elevation_override_xy)
    eta_override = cfg.surface_elevation_override_xy;
    if ~(isnumeric(eta_override) && isreal(eta_override) && ...
            isequal(size(eta_override), [cfg.ny, cfg.nx]) && all(isfinite(eta_override(:))))
        error(['surface_elevation_override_xy must be an ny-by-nx finite real ', ...
            'numeric matrix.']);
    end
    if ~any(strcmp(cfg.surface_boundary_model, {'kirchhoff_spatial', 'kirchhoff_kdomain'}))
        error(['surface_elevation_override_xy is supported only by ', ...
            'kirchhoff_spatial and kirchhoff_kdomain.']);
    end
    cfg.surface_elevation_override_xy = double(eta_override);
end
...
cfg.surface_ssa_frequency_correlation_mode = local_normalize_choice(cfg.surface_ssa_frequency_correlation_mode, 'surface_ssa_frequency_correlation_mode');
if ~any(strcmp(cfg.surface_ssa_frequency_correlation_mode, {'independent', 'shared_seed_phase', 'ar1_frequency'}))
    error('surface_ssa_frequency_correlation_mode must be ''independent'', ''shared_seed_phase'', or ''ar1_frequency''.');
end
if ~(isscalar(cfg.surface_ssa_frequency_correlation_rho) && isnumeric(cfg.surface_ssa_frequency_correlation_rho) && ...
     isfinite(cfg.surface_ssa_frequency_correlation_rho) && cfg.surface_ssa_frequency_correlation_rho >= 0 && ...
     cfg.surface_ssa_frequency_correlation_rho < 1)
    error('surface_ssa_frequency_correlation_rho must satisfy 0 <= rho < 1.');
end
...
if ~isscalar(cfg.surface_ssa_component_diagnostics)
    error('surface_ssa_component_diagnostics must be a scalar logical flag.');
end
cfg.surface_ssa_component_diagnostics = logical(cfg.surface_ssa_component_diagnostics);
if ~isscalar(cfg.surface_wavefield_diagnostics)
    error('surface_wavefield_diagnostics must be a scalar logical flag.');
end
cfg.surface_wavefield_diagnostics = logical(cfg.surface_wavefield_diagnostics);
cfg.surface_wavefield_slice_axis = local_normalize_choice( ...
    cfg.surface_wavefield_slice_axis, 'surface_wavefield_slice_axis');
if ~any(strcmp(cfg.surface_wavefield_slice_axis, {'x', 'y'}))
    error('surface_wavefield_slice_axis must be ''x'' or ''y''.');
end
if cfg.surface_wavefield_max_z_samples < 2
    error('surface_wavefield_max_z_samples must be at least 2.');
end
...
if ~isscalar(cfg.surface_kstat_random_scatter)
    error('surface_kstat_random_scatter must be a scalar logical flag.');
end
cfg.surface_kstat_random_scatter = logical(cfg.surface_kstat_random_scatter);
cfg.surface_kstat_seed_offset = local_force_int(cfg.surface_kstat_seed_offset, 'surface_kstat_seed_offset');
if cfg.surface_kstat_seed_offset < 0
    error('surface_kstat_seed_offset must be nonnegative.');
end
cfg.surface_kstat_conv_padding = local_normalize_choice(cfg.surface_kstat_conv_padding, 'surface_kstat_conv_padding');
if ~any(strcmp(cfg.surface_kstat_conv_padding, {'periodic', 'zero_padded'}))
    error('surface_kstat_conv_padding must be ''periodic'' or ''zero_padded''.');
end
if ~(isscalar(cfg.surface_kstat_trusted_angle_deg) && isnumeric(cfg.surface_kstat_trusted_angle_deg) && ...
     (isnan(cfg.surface_kstat_trusted_angle_deg) || ...
      (isfinite(cfg.surface_kstat_trusted_angle_deg) && cfg.surface_kstat_trusted_angle_deg >= 0 && cfg.surface_kstat_trusted_angle_deg <= 90)))
    error('surface_kstat_trusted_angle_deg must be NaN or a finite scalar in [0,90].');
end
...
if cfg.validation_allow_extended_sponge_ratio, ratio_limits=[0 0.20]; end
...
if ~isempty(output.R_center) && ~isempty(output.A_center)
    ...
    labels = {};
    if any(valid)
        h(end+1) = loglog(output.R_center(valid), output.A_center(valid), 'b-', 'LineWidth', 1.2);
        labels{end+1} = 'Measured center amplitude';
        hold on
    end
    if any(output.fit_mask)
        ...
        h(end+1) = loglog(R_fit, C_fit ./ R_fit, 'r--', 'LineWidth', 1.2);
        labels{end+1} = 'C/R fit';
    end
    grid on
    xlabel('R (m)')
    ylabel('|psi_{center}|')
