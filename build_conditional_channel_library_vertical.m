...
required={'frequency_axis','reference_delay_s','H_direct_f','H_ref_coh_f','stats','condition'};
...
f=models{1}.frequency_axis(:); tref=models{1}.reference_delay_s;
...
for jj=1:numel(models)
    ...
    if abs(m.reference_delay_s-tref)>1e-12
        error('All condition nodes must use the same reference delay.');
    end
...
library=struct('schema_version','1.0.0-discrete', ...
    'code_revision',local_option(options,'code_revision','uncommitted'), ...
    'frequency_axis',f,'reference_delay',tref, ...
